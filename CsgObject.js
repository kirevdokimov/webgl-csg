...
const vns = (vertices, normals) => ({ vertices, normals });
const elementsCount = (s) => s.vertices.length / 3; // По одному числу на каждую компоненту вектора
const mergeVertNormSequences = (s) => {
    let v = [];
    let n = [];
    s.forEach(vn => {
        v.push(...vn.vertices);
...
const triangulateConvexVertNormSequence = (s) => {
    ...
    for (let i = 2; i < numberOfElements; i++) {
        v = v.concat(s.vertices[0], s.vertices[1], s.vertices[2], // 0
...
const polygonToVertNormSequence = (p) => {
...
export const makeCsgObject = (gl, polygons) => {
    ...
    return {
        ...
        render: (time) => {
            ...
            gl.drawArrays(gl.TRIANGLES, 0, numberOfElements);
        }
    };
};
//# sourceMappingURL=CsgObject.js.map