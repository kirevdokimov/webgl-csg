...
import { mat3, mat4, vec3 } from "./gl-matrix/index.js";
import { RawControls } from "./RawControls.js";
...
export const makeCameraPanControls = (camera: Camera, controls: RawControls): PanControls => {
    ...
    const recalcFwdRight = (target: vec3, position: vec3) => {
        ...
        update: (time) => {
            if (controls.state.mouse.rmb) {
                ...
                { // ограничение по вращению верх-низ
                    ...
                    if (angle + ky > maxAngle) ky = maxAngle - angle
                    if (angle + ky < minAngle) ky = minAngle - angle
                }
            ...
            if (controls.state.mouse.wheel != 0) {
                ...
                if (from + moveFrom < minDistance) moveFrom = minDistance - from
                if (from + moveFrom > maxDistance) moveFrom = maxDistance - from
