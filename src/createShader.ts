export const createShader = (gl: WebGLRenderingContext, vs: string, fs: string): WebGLProgram => {
    const vertexShader = gl.createShader(gl.VERTEX_SHADER);
    if (!vertexShader) throw new Error("Fragment shader could not be created")
    ...
    if (!fragmentShader) throw new Error("Fragment shader could not be created")
    ...
    if (!prg) throw new Error("Shader program could not be created")
